...
void setup()
{
  ...
  while (millis() < waitTime)
    yield(); // wait one half second for the serial port
  Serial.println("\n\nAnalog Clock");
  ...
  while (Serial.available())
    Serial.read(); // empty the serial input buffer
  ...
  if (!NTP.setInterval(10, 600))
    Serial.println("Problem setting NTP interval.");
  NTP.begin(NTPSERVERNAME, true); // start the NTP client

  int waitCount = 500;
  Serial.print("Waiting for sync with NTP server");
  while (timeStatus() != timeSet)
  { // wait until the the time is set and synced
    waitTime = millis() + 500;
    while (millis() < waitTime)
      yield();         // wait one half second
    Serial.print("."); // print a "." every half second
    --waitCount;
    if (waitCount == 0)
      ESP.restart(); // if time is not set and synced after 50 seconds, restart the ESP8266
  }
...
void loop()
{
  ...
  if (printTime)
  { // when the analog clock is updated...
    printTime = false;
...
void pulseCoil()
{
  if ((analogClkSecond % 2) == 0)
  { // positive motor pulse on even seconds
    digitalWrite(COIL1, HIGH);
    digitalWrite(COIL2, LOW);
  }
  else
  { // negative motor pulse on odd seconds
    digitalWrite(COIL1, LOW);
...
void checkClock()
{
  ...
  { // if the analog clock is behind the actual time and needs to be advanced...
...
void updateClock()
{
  ...
  if (++analogClkSecond == 60)
  {                      // since the clock motor has been pulsed, increase the seconds count
...
String getUpTime()
{
  ...
  String daysStr = "";
  if (d > 0)
    (d == 1) ? daysStr = "1 day," : daysStr = String(d) + " days,";
  String hoursStr = "";
  if (h > 0)
    (h == 1) ? hoursStr = "1 hour," : hoursStr = String(h) + " hours,";
  String minutesStr = "";
  if (m > 0)
    (m == 1) ? minutesStr = "1 minute" : minutesStr = String(m) + " minutes";
